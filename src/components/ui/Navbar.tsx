                ...
                <div className="otherglass backdrop-blur-md rounded-md shadow-xl shadow-black/5 border-gray-700 overflow-hidden py-2 flex flex-col">
                  {projects.map((project) => (
                    <Link
                      ...
                      className="px-4 py-2.5 text-sm text-white hover:text-blue-logo transition-colors"
                    >
                      {project.title}
                    </Link>
                  ))}
          ...
          >
            <nav className="flex flex-col items-center py-6 gap-6 text-sm font-semibold">
              ...
              {hasMounted && (status !== "authenticated" ? (
                ...
                <Link onClick={() => setMobileMenuOpen(false)} href="/#contato" className="px-6 py-2 rounded-full bg-white text-brand-950 hover:bg-blue-logo hover:text-white transition-all transform hover:scale-105 active:scale-95 shadow-md text-center w-full">
                  Contato
                </Link>
              ))}
            </nav>
